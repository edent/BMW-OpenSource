...
static struct gpiomux_setting cdc_i2s_mclk = {
	.func = GPIOMUX_FUNC_1,
/*@peiker/dl 05.09.2013 A CQCM00028073 Reduce drive strength to 2mA */
//	.drv = GPIOMUX_DRV_8MA,
	.drv = GPIOMUX_DRV_2MA,
/*@peiker/dl 05.09.2013 E CQCM00028073 Reduce drive strength to 2mA */
 ...
 */
#ifdef PEIKER_CODEC
/* PA enable on GPIO 78 */
static u32 pa_enable = 78;
#else
...
#ifdef PEIKER_CODEC
        snd_soc_dapm_enable_pin(dapm, "External PA");
	} else {
        snd_soc_dapm_disable_pin(dapm, "External PA");
#else
...
#ifdef PEIKER_CODEC
    if (!strncmp(w->name, "External PA", 11)) {
        if (SND_SOC_DAPM_EVENT_ON(event)) {
            /* @Peiker/QC - enable external PA via gpio */
			ret = gpio_request(pa_enable, "PA_ENABLE");
			if (ret) {
				pr_err("%s: Error requesting PA_ENABLE %u\n",
					__func__, pa_enable);
				return -EINVAL;
			}
            ret = gpio_direction_output(pa_enable, 1);
            if (ret) {
                pr_err("%s: Error setting PA_ENABLE %u\n",
                    __func__, pa_enable);
                return -EIO;
            }
        } else {

            /* @Peiker/QC - Disable PA and free GPIO */
            gpio_direction_output(pa_enable, 0);
            gpio_free(pa_enable);
        }
    }
    else {
...
#ifdef PEIKER_CODEC
   SND_SOC_DAPM_SUPPLY("External PA",  SND_SOC_NOPM, 0, 0,
                        mdm9615_spkramp_event, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
};

static const struct snd_soc_dapm_route common_audio_map[] = {
    /* Setup DAC path dependency on external PA */
    {"Left DAC", NULL, "External PA"},
#else
...
static const char *rx_ch_text[] = {"One", "Two"};
static const char *tx_ch_text[] = {"One", "Two", "Three", "Four"};
...
static const struct snd_kcontrol_new msm9615_i2s_controls[] = {
...
static int msm9615_i2s_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	struct snd_soc_dapm_context *dapm = &codec->dapm;
	err = snd_soc_add_controls(codec, msm9615_i2s_controls,
		ARRAY_SIZE(msm9615_i2s_controls));
	if (err < 0) {
				       ...
				       enum msm9x15_set_i2s_clk enable)
{
	...
	if (enable == MSM_I2S_CLK_SET_TRUE ||
		...
		clk_set_rate(clk_ctl->osr_clk, EXT_CLK_RATE);
				 ...
				 struct snd_pcm_hw_params *params)
{
    ...
    /* @Peiker/QC - Set static mclk for both playback and capture.
     * This clk has already been enabled in msm9615_i2s_startup() but
     * we need to inform the codec here so it doesn't try to enable internal
     * PLL. */
    snd_soc_dai_set_sysclk(rtd->codec_dai, 0, EXT_CLK_RATE, SND_SOC_CLOCK_OUT);
    /* @Peiker/QC - end */
...
static struct snd_soc_dai_link mdm9615_dai_common[] = {
    ...
    /* Backend I2S DAI Links */
    {
        ...
        .platform_name = "msm-pcm-routing",
        .codec_name     = "tlv320aic3x.0-0018",
        .codec_dai_name	= "tlv320aic3x_rx1",
        .no_pcm = 1,
    ...
    },
    {
        ...
        .platform_name = "msm-pcm-routing",
        .codec_name     = "tlv320aic3x.0-0018",
        .codec_dai_name	= "tlv320aic3x_tx1",
        .no_pcm = 1,
...
static struct snd_soc_card snd_soc_card_mdm9615 = {
		//.name       = "tlv320aic3x-snd-card", // String match here?!?!
        .name       = "tlv320aic3x", // String match here?!?!
};
...
#ifdef PEIKER_CODEC
	/* TLV320AIC3X only supports I2S, no need to detect */
