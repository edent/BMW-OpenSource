	...
	while (page_count-- > 0) {
		...
		for (n = start_sector; n < cwperpage; n++) {
			...
			if (ops->datbuf) {
				if (ops->mode != MTD_OOB_RAW)
				{
				  if (chip->nand_mode == NAND_MODE_MODEM){
	          sectordatasize = 512;
				  }else{
		...
		if (rawerr && !pageerr) {
			...
			for (n = start_sector; n < cwperpage; n++)
				pr_info("rawerr set, -> flash_status[%d] = %x,\
				buffr_status[%d] = %x\n",
 ...
 */
static nand_mode_e_t nand_scan_mode = NAND_MODE_LINUX;
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
  ...
  chip->nand_mode=nand_scan_mode;
  if (NAND_MODE_LINUX == nand_scan_mode ){
    ...
    chip->CFG0 = (((mtd_writesize >> 9)-1) << 6) /* 4/8 cw/pg for 2/4k */
      ...
      |    (0 << 30)  /* Do not read status before data */
      |    (1 << 31)  /* Send read cmd */
      /* 4 spare bytes for 16 bit nand or 5 spare bytes for 8 bit */
      | (wide_bus ?  (4 << 23) : (5 << 23));
...
#define LOCAL_MAX_NAME_SIZE 1024
static int setup_mtd_device_m(struct platform_device *pdev,
           struct msm_nand_info *info)
{
  char *modified_part_names, *curr_p;
  struct mtd_partition *copied_parts;
  int i, err, name_size, curr_len;
  struct flash_platform_data *pdata = pdev->dev.platform_data;

  if (pdata){
    /* this will add an 'm' character to the front of all partition names provided */
    copied_parts = kzalloc((sizeof(struct mtd_partition))*pdata->nr_parts,GFP_KERNEL);
    modified_part_names = kzalloc (LOCAL_MAX_NAME_SIZE,GFP_KERNEL);
    if ((NULL == copied_parts)|| (NULL == modified_part_names))
    {
      pr_err("%s: no memory for temporary partition name copy\n", __func__);
      kfree (copied_parts);
      kfree (modified_part_names);
      err = -ENOMEM;
    }else
    {
      memcpy (copied_parts,&pdata->parts[0],(sizeof(struct mtd_partition))*pdata->nr_parts);
      curr_p = modified_part_names;
      name_size = 0;
      for (i = 0; i < pdata->nr_parts; i++) {
        curr_len = strlen(pdata->parts[i].name)+2;
        name_size += curr_len;
        if (name_size < LOCAL_MAX_NAME_SIZE){
          copied_parts[i].name = curr_p;
          *curr_p = 'm';
          strcpy ((curr_p+1),pdata->parts[i].name);
          curr_p += curr_len;
        }
      }
      err = mtd_device_register(&info->mtd, copied_parts,
          pdata->nr_parts);
      kfree (copied_parts);
      kfree (modified_part_names);
    }
...
/* @PEIKER/JKOM 08.11.2012 A  extra mtd devices to access modem partitions */
  info = kzalloc(sizeof(struct msm_nand_info), GFP_KERNEL);
  if (!info) {
    pr_err("%s: No memory for msm_nand_info\n", __func__);
    return -ENOMEM;
  }
  nand_scan_mode = NAND_MODE_MODEM;
  info->msm_nand.dev = &pdev->dev;
  ...
  err = setup_mtd_device_m(pdev, info);
  if (err < 0) {
    pr_err("%s: setup_mtd_device failed with err=%d\n",
        __func__, err);
    goto out_free_dma_buffer;
  }
  nand_scan_mode = NAND_MODE_LINUX;
/* @PEIKER/JKOM 08.11.2012 E  extra mtd devices to access modem partitions*/
	return 0;
