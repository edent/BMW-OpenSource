...
static uint data_pad_ctl = 0x034A5610;
static uint strobe_pad_ctl = 0x034A5610;

module_param(data_pad_ctl, uint, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
MODULE_PARM_DESC(data_pad_ctl, "HSIC-Data PAD control");

module_param(strobe_pad_ctl, uint, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
MODULE_PARM_DESC(strobe_pad_ctl, "HSIC-Strobe PAD control");
...
#define HSIC_STB_PAD_CTL	(MSM_TLMM_BASE+0x20CC)
...
static int msm_hsic_reset(struct msm_hsic_hcd *mehci)
{
	...
	if (pdata && pdata->strobe && pdata->data) {
		...
		/* pad control */
		writel_relaxed(strobe_pad_ctl, HSIC_STB_PAD_CTL);
		dev_info(mehci->dev, "HSIC_STB_PAD_CTL: 0x%08X\n", strobe_pad_ctl);

		writel_relaxed(data_pad_ctl, HSIC_DATA_PAD_CTL);
		dev_info(mehci->dev, "HSIC_DATA_PAD_CTL: 0x%08X\n", data_pad_ctl);
...
static int msm_hsic_pm_resume(struct device *dev)
{
	...
	if (!atomic_read(&mehci->pm_usage_cnt) &&
		...
		dev_err(dev, "HSIC is resumed later when remote wakeup is received or interface driver start I/O");
		return 0;
	}
