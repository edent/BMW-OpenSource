...
static void msm_hsl_set_baud_rate(struct uart_port *port, unsigned int baud)
{
	 ...
	 * Hence configuring Rx Watermark as 6 Words (24 bytes)
	 */
	watermark = (port->fifosize * 3) / (4*4*2);
...
static int __devinit msm_serial_hsl_probe(struct platform_device *pdev)
{
	...
	/* Change UART_DM_BADR only for UART port 1 only */
	if (port->line == 1) {
		/* Read UART_DM_BADR register */
		rx_base_val = msm_hsl_read(port,
			regmap[msm_hsl_port->ver_id][UARTDM_BADR]);
		pr_info("%s(): rx_base_val:%d\n", __func__, rx_base_val);
		/* Set UART_DM_BADR register with value as 32 */
		msm_hsl_write(port, 32,
			regmap[msm_hsl_port->ver_id][UARTDM_BADR]);
		rx_base_val = msm_hsl_read(port,
			regmap[msm_hsl_port->ver_id][UARTDM_BADR]);
		pr_info("%s(): rx_base_val:%d\n", __func__, rx_base_val);
