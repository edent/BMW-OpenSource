...
static void lpass_fatal_fn(struct work_struct *work)
{
	...
	/* When the modem crashes lpass will notice and notify the apps proc
	 * that it is resetting. This results in a kernel panic.
	 * If CONFIG_MSM_CRASHDEBUG is compiled in we instead ignore this
	 * so we can finish dumping memory over ethernet.
	 * (we already know the system is unstable and will be reset) */
