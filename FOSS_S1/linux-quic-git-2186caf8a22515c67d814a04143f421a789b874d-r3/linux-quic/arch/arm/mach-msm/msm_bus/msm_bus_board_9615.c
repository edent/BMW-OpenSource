	...
	*fabreg, int fabid)
{
	...
	for (i = 0; i < fabreg->len; i++) {
        ...
        if(fabreg->info[i].id < NMASTERS) {
				  master_iids[fabreg->info[i].id] =
					  fabreg->info[i].priv_id;
        }
        else {
          MSM_BUS_ERR("No memory for master_iids index 0%x\n", fabreg->info[i].id);
        }
      }
			else {
        index = fabreg->info[i].id - (SLAVE_ID_KEY);
        if(index < NSLAVES) {
				  slave_iids[index]
					= fabreg->info[i].priv_id;
        }
        else {
         	MSM_BUS_ERR("No memory for slave_iids index 0%x\n", index);
        }
		  }
    }
    else
